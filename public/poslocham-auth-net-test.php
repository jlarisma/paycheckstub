...
//    include __DIR__."/pdf_render-s2.php";
      include __DIR__."/pdf_render.php";
    return;
}
...
<?php
    ...
    switch ($AVS)
    {
		...
		// this gets the first result... but.. maybe here we can get all results from the user.
		$trx_options = json_decode(base64_decode(base64_encode(json_encode($trx['options']))), true);  
		//$trx_options = $trx['options']);  
		//$trx_options = base64_decode($trx['options']); 		// all the serialized data from $trx into $trx_options
		//print_r(base64_decode($trx['options']));
		print_r($trx_options);
...
$pageList = '';
if ($Amount == 4.50){ $trx_options['is_customer_paid1'] = 1;   }
else {
