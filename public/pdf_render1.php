...
//echo ($content_pdf);
	$mail-> AddStringAttachment ($content_pdf, "{$pageStyle}_Paystub_check_$ix.pdf", 'base64', 'application/pdf');
}
...
$to = $email;
$mail->AddAddress($to);
$mail->Subject = "Your {$pageStyle} Banks Statement From Paycheck Stub online";
$mail->AltBody = "To view the message, please use an HTML compatible email viewer!"; // optional, comment out and test
$mail->WordWrap = 80;                                               // set word wrap
$mail->MsgHTML($body);
$mail->IsHTML(true); // send as HTM
...
echo"</pre>";
    ...
    $content_pdf = file_get_contents($pdf->getPdfFilename());
//echo ($content_pdf);
	$mail-> AddStringAttachment ($content_pdf, "BanksStatement_$ix.pdf", 'base64', 'application/pdf');
}
