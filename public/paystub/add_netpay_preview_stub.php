...
// HOURS AND PAY AREA
 ...
 $neat_gross_ot_rate = number_format(1.5 * $gross_rate + 0, 2) . "/hr";

 $ot_hrs_base =  _R('ot_hrs');                        //GROSS OVERTIME
$val_hrs = $val_hrs_base =  $gross_hrs_base = _R('gross_hrs');                               //Calculations
$val_rate_base = $gross_rate_base = _R('gross_rate');                             //Calculations
$commission_prd_base = $val_commission_prd_base = _R('commission_prd');

$val_ot_hrs = _R('ot_hrs_');                
/* @var $ot_hrs type */
$ot_hrs = $val_ot_hrs[$prd_num];       //GROSS OVERTIME
             $gross_hrs = $val_hrs[$prd_num] ;   //Calculations

$gross_hrs *= $val_hourly_multiplier;
$val_rate = _R('gross_rate_');               $gross_rate = $val_rate [$prd_num];  //Calculations
$val_commission_prd = _R('commission_prd_'); 
$commission_prd =$val_commission_prd[$prd_num];
$val_commission_prd = $commission_prd;
...
if($status == 3){
 ...
 $commission_ytd = $val_commission_ytd = _R('commission_ytd');
...
$val_fed_tax_ytd= ($val_fed_tax)*($val_multiplier)*($payprds_completed)+(.25 * $val_commission_ytd);                           //calculations      (calculated-fedtax)  * (weeks)
 ...
 $fed_amtytd = (_R('fed_amtytd')); // =$val_fed_tax_ytd
 $fedr_with_held_ytd = ($val_fed_tax_ytd);
 $fed_amt_deduct_ytd = ($val_fed_tax_ytd);

registerN2vars("commission_ytd", 'fed_amt_deduct_prd', 'fed_amtytd', 'fedr_with_held_ytd', 'fed_amt_deduct_ytd',
